...
class LogGenerator:
    ...
    def generate_log(self):
        ...
        soil_humidity = self.raspbi_controller.read_soil_humidity()    # Humedad del suelo (%)
        air_humidity, temperature = self.raspbi_controller.read_dht11()  # Humedad del aire (%) y temperatura (°C)
        ...
        pot_data = PotData(soil_humidity=soil_humidity,
        ...
        log_entry = Log(pot=self.pot, data=pot_data, advice=expert_advice, irrigation=irrigation_event)
    ...
    def save_log(self):
        ...
        """
        if self.log is None:
            print("No hay ningún log generado para guardar. Por favor, genere el log primero.")
            return
        ...
        print(f"Guardando el log de la maceta '{self.pot}' en la base de datos... (simulación)")
