...
class PotRepository(Repository):
    ...
    def get_all_logs(self, pot):
        ...
        for log in logs_dict:
            if pot.id == log["id_pot"]:
                new = Log(log["id"],log["id_log"],log["id_plant"],log["temperature"],log["soil_humidity"],log["air_humidity"],log["image_path"],log["expert_advice"])
                pot.add_log(new)
            else:
    ...
    def get_last_log(self, pot):
        ...
        if pot.id == log["id"]:
            last_log =Log(log["id"],log["id_log"],log["id_plant"],log["temperature"],log["soil_humidity"],log["air_humidity"],log["image_path"],log["expert_advice"])
        else: 
            print("Este log no pertenece al pot, Acceso cukatrap")
        return last_log
